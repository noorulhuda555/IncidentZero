...
|--------|-----------|------------------|-------------------|-------|
| [ ] | `incidentzero/agent/controller.py:48` | `AgentController._model_decide` | Integrate bounded model retries, budget accounting, and trace metrics around LLM calls | 7 (also 5, 8) |
...
| [ ] | `incidentzero/agent/recovery.py:18` | `RetryPolicy.call_model` | Bounded backoff retry for `TransientModelError` only | 5, 7 |

**Total open (code markers):** 3 — run `python scripts/count_todos.py`
